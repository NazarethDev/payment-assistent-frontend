...
export async function updateBill(id, bill) {
    return paymentAssistentApi.patch(`${API_BASE_URL}${UPDATE_BILL}/${id}`, bill);
};

export async function deleteBill(id) {
    return paymentAssistentApi.delete(`${API_BASE_URL}${DELETE_BILL}/${id}`);
};

export async function getTodayBills() {
    return paymentAssistentApi.get(`${API_BASE_URL}${GET_TODAY_BILLS}`);
};
